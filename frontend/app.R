...
server <- function(input, output, session) {
  ...
  output$export_html_report <- downloadHandler(
    ...
    content = function(file) {
      # Get current data based on what user is viewing
      viz_data <- viz_performance_data()
      
      if (nrow(viz_data) == 0) {
        # Create simple error page if no data
        error_html <- '
<!DOCTYPE html>
<html>
<head><title>Benchmarking Report</title></head>
<body>
  <h1>No Data Available</h1>
  <p>No experiments selected for export.</p>
</body>
</html>'
      ...
      # Generate HTML content using viz_data (which contains everything we need)
      html_content <- generate_html_export(viz_data)  # This function is defined at the bottom
...
<body>
  ...
  <p>No experiments selected for export.</p>
          ...
          html_content <- generate_html_export(viz_data)
          
          # Write to file for download
          writeLines(html_content, file)
          
        }, error = function(e) {
          # If there's an error, create a simple error report
          error_html <- paste0('
<!DOCTYPE html>
<html>
...
<body>
  ...
  # =============================================================================
  # HTML GENERATION FUNCTION (place this BEFORE shinyApp call, not inside server)
  # =============================================================================
  
  generate_html_export <- function(viz_data) {
    
    # Helper function to safely convert to numeric and multiply by 100
    ...
    experiment_ids <- unique(viz_data$experiment_id)
    experiment_ids <- experiment_ids[!is.na(experiment_ids)]
...
<body>
    ...
    html_content <- paste0(html_content, '
        <h2>SNP Performance Results (Best to Worst F1 Score)</h2>')
    ...
    if (nrow(snp_data) > 0) {
      ...
      html_content <- paste0(html_content, '<p>No SNP data available</p>')
    }
    
    # INDEL Results Table
    html_content <- paste0(html_content, '
        <h2>INDEL Performance Results (Best to Worst F1 Score)</h2>')
    ...
    if (nrow(indel_data) > 0) {
      ...
      html_content <- paste0(html_content, '<p>No INDEL data available</p>')
    }
    ...
    # Add metadata for each experiment
    for (exp_id in experiment_ids) {
      exp_data <- viz_data[viz_data$experiment_id == exp_id, ]
      if (nrow(exp_data) > 0) {
        exp_data <- exp_data[1, ]  # Get first row for this experiment
        
        # Safe coverage formatting
        coverage_text <- if(is.na(exp_data$mean_coverage) || is.null(exp_data$mean_coverage)) {
          "N/A"
        } else {
          tryCatch({
            paste0(round(as.numeric(exp_data$mean_coverage), 1), "x")
        ...
        <div class="metadata-section">
            ...
            <div class="metadata-grid">
                <div>
                    ...
                    <div class="metadata-item"><strong>Coverage:</strong> ', coverage_text, '</div>
                </div>
                <div>
