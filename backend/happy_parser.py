...
def parse_happy_csv(happy_file_name, experiment_id, session):
   ...
   try:
       ...
       existing_benchmark = session.query(BenchmarkResult).filter_by(
           experiment_id=experiment_id
       ).first()
       
       existing_overall = session.query(OverallResult).filter_by(
           experiment_id=experiment_id
       ).first()
       ...
       for _, row in filtered_df.iterrows():
           ...
           # Store in overall table (ALL subset only)
